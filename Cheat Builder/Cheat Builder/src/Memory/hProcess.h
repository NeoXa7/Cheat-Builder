...
// getting process infos; 
DWORD GetProcessInfo::GetProcessID(const wchar_t* processName) {
    DWORD processID = 0;
...
uintptr_t GetProcessInfo::GetModuleBaseAddress(DWORD processId, const wchar_t* ModuleTarget) {
...
void GetProcessInfo::CheckProcess(DWORD procID, const wchar_t* processName)
{
    if (procID == 0)
    {
...
void GetProcessInfo::CheckBaseAddress(uintptr_t baseAddress)
{
    if (baseAddress == 0)
    {
